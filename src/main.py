...
def place(bot, update):
    ...
    try:
        # TODO: account for user name changes
        game_handler.place_stone(chat_id, (user_id, user_name), coords)
        show_board(bot, update)
...
def show_board(bot, update):
    chat_id = update.message.chat_id
    _, cur_player_name = game_handler.cur_player(chat_id)
    image = game_handler.create_image(chat_id)
    bot.send_photo(chat_id, photo=image)
    # TODO: extract string
    send_message(bot, chat_id, f"It is {cur_player_name}'s turn")
